...
(function(t, u, l) {
    ...
    function U(c, g) {
        var e = [];
        return {storage: e,width: c,height: g,fillRect: function() {
                e.push({type: "function",name: "fillRect",arguments: arguments})
            },drawImage: function() {
                e.push({type: "function",name: "drawImage",arguments: arguments})
            },fillText: function() {
                e.push({type: "function",name: "fillText",arguments: arguments})
            },setVariable: function(a, d) {
                e.push({type: "variable",name: a,arguments: d})
            }}
    }
    var i = {}, T, L;
    i.Util = {};
    i.Util.backgroundImage = 
    function(c) {
        if (/data:image\/.*;base64,/i.test(c) || /^(-webkit|-moz|linear-gradient|-o-)/.test(c))
            return c;
    ...
    i.Util.Bounds = function(c) {
        ...
        if (c.getBoundingClientRect)
            return c = c.getBoundingClientRect(), g.top = c.top, g.bottom = c.bottom || c.top + c.height, g.left = c.left, g.width = c.width || c.right - c.left, g.height = c.height || c.bottom - c.top, g
    };
    i.Util.getCSS = function(c, g) {
        function e(a, 
        b) {
            var g = c.runtimeStyle && c.runtimeStyle[a], e, k = c.style;
        ...
        if (t.getComputedStyle)
            void 0 !== c && (T = u.defaultView.getComputedStyle(c, null)), a = T[g], g === "backgroundPosition" && (a = (a.split(",")[0] || "0 0").split(" "), a[0] = a[0].indexOf("%") === 
            -1 ? e(g + "X", a[0]) : a[0], a[1] = a[1] === l ? a[0] : a[1], a[1] = a[1].indexOf("%") === -1 ? e(g + "Y", a[1]) : a[1]);
        else if (c.currentStyle)
            if (g === "backgroundPosition")
                a = [e(g + "X", c.currentStyle[g + "X"]), e(g + "Y", c.currentStyle[g + "Y"])];
            else if (a = e(g, c.currentStyle[g]), /^(border)/i.test(g) && /^(medium|thin|thick)$/i.test(a))
                switch (a) {
                    case "thin":
                        a = "1px";
                        break;
    ...
    i.Util.BackgroundPosition = function(c, g, e) {
        var c = i.Util.getCSS(c, "backgroundPosition"), a, d;
        c.length === 1 && (a = 
        c, c = [], c[0] = a, c[1] = a);
    ...
    (function() {
        i.Generate = {};
        var c = [/^(-webkit-linear-gradient)\(([a-z\s]+)([\w\d\.\s,%\(\)]+)\)$/, /^(-o-linear-gradient)\(([a-z\s]+)([\w\d\.\s,%\(\)]+)\)$/, /^(-webkit-gradient)\((linear|radial),\s((?:\d{1,3}%?)\s(?:\d{1,3}%?),\s(?:\d{1,3}%?)\s(?:\d{1,3}%?))([\w\d\.\s,%\(\)-]+)\)$/, 
            /^(-moz-linear-gradient)\(((?:\d{1,3}%?)\s(?:\d{1,3}%?))([\w\d\.\s,%\(\)]+)\)$/, /^(-webkit-radial-gradient)\(((?:\d{1,3}%?)\s(?:\d{1,3}%?)),\s(\w+)\s([a-z-]+)([\w\d\.\s,%\(\)]+)\)$/, /^(-moz-radial-gradient)\(((?:\d{1,3}%?)\s(?:\d{1,3}%?)),\s(\w+)\s?([a-z-]*)([\w\d\.\s,%\(\)]+)\)$/, /^(-o-radial-gradient)\(((?:\d{1,3}%?)\s(?:\d{1,3}%?)),\s(\w+)\s([a-z-]+)([\w\d\.\s,%\(\)]+)\)$/];
        i.Generate.parseGradient = function(g, e) {
            var a, d, b = c.length, h, f, k, n;
            for (d = 0; d < b; d += 1)
                if (h = g.match(c[d]))
                    break;
            if (h)
                switch (h[1]) {
                    ...
                    case "-o-linear-gradient":
                        ...
                        if (a.x0 === null && a.x1 === null)
                            a.x0 = a.x1 = e.width / 2;
                        if (a.y0 === null && a.y1 === null)
                            a.y0 = a.y1 = e.height / 2;
                        if (b = h[3].match(/((?:rgb|rgba)\(\d{1,3},\s\d{1,3},\s\d{1,3}(?:,\s[0-9\.]+)?\)(?:\s\d{1,3}(?:%|px))?)+/g)) {
                            f = b.length;
                            k = 1 / Math.max(f - 
                            1, 1);
                            for (d = 0; d < f; d += 1)
                                n = b[d].match(/((?:rgb|rgba)\(\d{1,3},\s\d{1,3},\s\d{1,3}(?:,\s[0-9\.]+)?\))\s*(\d{1,3})?(%|px)?/), n[2] ? (h = parseFloat(n[2]), h /= n[3] === "%" ? 100 : e.width) : h = d * k, a.colorStops.push({color: n[1],stop: h})
                        }
                        break;
                    case "-webkit-gradient":
                        a = {type: h[2] === "radial" ? "circle" : h[2],x0: 0,y0: 0,x1: 0,y1: 0,colorStops: []};
                        if (b = h[3].match(/(\d{1,3})%?\s(\d{1,3})%?,\s(\d{1,3})%?\s(\d{1,3})%?/))
                            a.x0 = b[1] * e.width / 100, a.y0 = b[2] * e.height / 100, a.x1 = b[3] * e.width / 100, a.y1 = b[4] * e.height / 100;
                        if (b = h[4].match(/((?:from|to|color-stop)\((?:[0-9\.]+,\s)?(?:rgb|rgba)\(\d{1,3},\s\d{1,3},\s\d{1,3}(?:,\s[0-9\.]+)?\)\))+/g)) {
                            f = 
                            b.length;
                            for (d = 0; d < f; d += 1)
                                n = b[d].match(/(from|to|color-stop)\(([0-9\.]+)?(?:,\s)?((?:rgb|rgba)\(\d{1,3},\s\d{1,3},\s\d{1,3}(?:,\s[0-9\.]+)?\))\)/), h = parseFloat(n[2]), n[1] === "from" && (h = 0), n[1] === "to" && (h = 1), a.colorStops.push({color: n[3],stop: h})
                        }
                        break;
                    case "-moz-linear-gradient":
                        a = {type: "linear",x0: 0,y0: 0,x1: 0,y1: 0,colorStops: []};
                        if (b = h[2].match(/(\d{1,3})%?\s(\d{1,3})%?/))
                            a.x0 = b[1] * e.width / 100, a.y0 = b[2] * e.height / 100, a.x1 = e.width - a.x0, a.y1 = e.height - a.y0;
                        if (b = h[3].match(/((?:rgb|rgba)\(\d{1,3},\s\d{1,3},\s\d{1,3}(?:,\s[0-9\.]+)?\)(?:\s\d{1,3}%)?)+/g)) {
                            f = 
                            b.length;
                            k = 1 / Math.max(f - 1, 1);
                            for (d = 0; d < f; d += 1)
                                n = b[d].match(/((?:rgb|rgba)\(\d{1,3},\s\d{1,3},\s\d{1,3}(?:,\s[0-9\.]+)?\))\s*(\d{1,3})?(%)?/), n[2] ? (h = parseFloat(n[2]), n[3] && (h /= 100)) : h = d * k, a.colorStops.push({color: n[1],stop: h})
                        }
                        break;
                    case "-webkit-radial-gradient":
                    case "-moz-radial-gradient":
                    case "-o-radial-gradient":
                        a = {type: "circle",x0: 0,y0: 0,x1: e.width,y1: e.height,cx: 0,cy: 0,rx: 0,ry: 0,colorStops: []};
                        if (b = h[2].match(/(\d{1,3})%?\s(\d{1,3})%?/))
                            a.cx = b[1] * e.width / 100, a.cy = b[2] * e.height / 100;
                        b = h[3].match(/\w+/);
                        n = h[4].match(/[a-z-]*/);
                        if (b && n)
                            switch (n[0]) {
                                ...
                                case "closest-corner":
                                    d = Math.sqrt(Math.pow(a.cx, 2) + Math.pow(a.cy, 2));
                                    b = Math.sqrt(Math.pow(a.cx, 2) + Math.pow(a.y1 - a.cy, 2));
                                    f = Math.sqrt(Math.pow(a.x1 - a.cx, 2) + Math.pow(a.y1 - a.cy, 
                                    2));
                                    n = Math.sqrt(Math.pow(a.x1 - a.cx, 2) + Math.pow(a.cy, 2));
        ...
        i.Generate.Gradient = function(c, e) {
            var a = u.createElement("canvas"), d = a.getContext("2d"), b, h, f, k, n;
            a.width = e.width;
            a.height = e.height;
            ...
            if (b)
                if (b.type === "linear") {
                    ...
                    f = 
                    0;
                    for (k = b.colorStops.length; f < k; f += 1)
                        try {
                    ...
                    var v = u.createElement("canvas"), x = v.getContext("2d");
        ...
        i.Generate.ListAlpha = function(c) {
            var e = "", a;
            do
                a = c % 26, e = String.fromCharCode(a + 64) + e, c /= 26;
            while (c * 26 > 26);
            return e
        };
        i.Generate.ListRoman = function(c) {
            var e = ["M", "CM", "D", "CD", "C", "XC", "L", "XL", "X", "IX", "V", "IV", "I"], a = [1E3, 900, 500, 400, 100, 90, 50, 40, 10, 9, 5, 4, 1], d = "", b, h = e.length;
            if (c <= 
            0 || c >= 4E3)
                return c;
            for (b = 0; b < h; b += 1)
    ...
    i.Parse = function(c, g) {
        function e() {
            return {width: Math.max(Math.max(z.body.scrollWidth, z.documentElement.scrollWidth), Math.max(z.body.offsetWidth, z.documentElement.offsetWidth), Math.max(z.body.clientWidth, z.documentElement.clientWidth)),height: Math.max(Math.max(z.body.scrollHeight, z.documentElement.scrollHeight), Math.max(z.body.offsetHeight, z.documentElement.offsetHeight), Math.max(z.body.clientHeight, z.documentElement.clientHeight))}
        }
        function a(a, b) {
        ...
        function f(a, c, e) {
            var e = e.ctx, f = y(a, 
            "fontFamily"), j = y(a, "fontSize"), k = y(a, "color"), n = y(a, "textDecoration"), F = y(a, "textAlign"), p = y(a, "letterSpacing"), I, q, m = y(a, "fontWeight"), r = y(a, "fontStyle"), s = y(a, "fontVariant"), A = 0, o;
            ...
            if (h(c.nodeValue).length > 0) {
                if (n !== "none")
                    if (w[f + "-" + j] !== l)
                        ...
                        a = {baseline: v,lineWidth: 1,middle: a.offsetTop - 
                            q.offsetTop + 1};
                        w[f + "-" + j] = a;
                        O.removeChild(q);
                        q = a
                ...
                for (f = F.length; m < f; m += 1) {
                    ...
                    if (r !== null)
                        s = I.left, p = I.bottom, a = e, h(r).length > 0 && (a.fillText(r, s, p), H += 1);
                    switch (n) {
                        case "underline":
                            d(e, 
                            I.left, Math.round(I.top + q.baseline + q.lineWidth), I.width, 1, k);
                            break;
                        case "overline":
        ...
        function n(a, b) {
            var c = Math.max(a.left, b.left), d = Math.max(a.top, b.top);
            return {left: c,top: d,width: Math.min(a.left + a.width, b.left + b.width) - c,height: Math.min(a.top + a.height, b.top + b.height) - d}
        }
        function Q(b, c, e, g) {
            for (var f = e.left, j = 
            e.top, h = e.width, e = e.height, k, p, B, q, m, i = function(b) {
                var c = [], d = ["Top", "Right", "Bottom", "Left"], e;
                for (e = 0; e < 4; e += 1)
                    c.push({width: a(b, "border" + d[e] + "Width"),color: y(b, "border" + d[e] + "Color")});
                return c
            }(b), b = 0; b < 4; b += 1)
                if (k = i[b], k.width > 0) {
                    p = f;
                    ...
                    q = {left: p,top: B,width: q,height: m};
                    g && (q = n(q, g));
                    q.width > 0 && q.height > 0 && d(c, p, B, q.width, q.height, 
                    k.color)
                }
            return i
        }
        function P(a, b, c) {
            var d = z.createElement("valuewrap"), e = ["lineHeight", "textAlign", "fontFamily", "color", "fontSize", "paddingLeft", "paddingTop", "width", "height", "border", "borderLeftWidth", "borderTopWidth"], g, j, h;
            g = 0;
        ...
        function x(a, b, c, d, e, g, f) {
            var f = Math.min(b.height, f), h, j;
            c.left -= Math.ceil(c.left / b.width) * b.width;
            for (j = d + c.left; j < g + d; )
                h = Math.floor(j + b.width) > 
                g + d ? g + d - j : b.width, v(a, b, j, e + c.top, h, f, d, e), j = Math.floor(j + b.width)
        }
        function K(b, c) {
            var j = i.Util.Bounds(b), f = j.left, B = j.top, w = j.width, D = j.height, F, p = y(b, "backgroundColor"), t = y(b, "position"), q, m = y(b, "opacity"), r, s;
            c ? L = {} : (L = e(), c = {opacity: 1});
            s = y(b, "zIndex");
            (q = c.zIndex) ? s !== "auto" && (s = {zindex: s,children: []}, q.children.push(s), q = s) : q = s = {zindex: 0,children: []};
            r = {ctx: U(L.width || w, L.height || D),zIndex: q,opacity: m * c.opacity,cssPosition: t};
            if (c.clip)
                r.clip = i.Util.Extend({}, c.clip);
            if (g.useOverflow === !0 && 
            /(hidden|scroll|auto)/.test(y(b, "overflow")) === !0 && /(BODY)/i.test(b.nodeName) === !1)
                r.clip = r.clip ? n(r.clip, j) : j;
            t = q.children.push(r);
            ...
            w = {left: f + m[3].width,top: B + m[0].width,width: w - (m[1].width + m[3].width),height: D - (m[0].width + m[2].width)};
            r.clip && (w = n(w, r.clip));
            if (w.height > 0 && w.width > 0) {
                d(s, w.left, w.top, 
                w.width, w.height, p);
                var C = w, o = y(b, "backgroundImage"), l = y(b, "backgroundRepeat").split(",")[0], u, G, J;
                !/data:image\/.*;base64,/i.test(o) && !/^(-webkit|-moz|linear-gradient|-o-)/.test(o) && (o = o.split(",")[0]);
                if (typeof o !== "undefined" && /^(1|none)$/.test(o) === !1)
                    if (o = i.Util.backgroundImage(o), p = k(o), D = i.Util.BackgroundPosition(b, C, p), p)
                        switch (l) {
                            case "repeat-x":
                            ...
                            case "repeat-y":
                                ...
                                D.top -= Math.ceil(D.top / p.height) * 
                                p.height;
                                for (J = l + D.top; J < u + l; )
                                    C = Math.floor(J + p.height) > u + l ? u + l - J : p.height, v(s, p, o + D.left, J, G, C, o, l), J = Math.floor(J + p.height);
                                break;
                            case "no-repeat":
                                o = C.width - D.left;
                                J = C.height - D.top;
                            ...
                            default:
                                ...
                                for (o = C.top + D.top; o < C.height + C.top; )
                                    l = Math.min(p.height, C.height + C.top - o), l = Math.floor(o + p.height) > l + o ? l + o - o : p.height, o < C.top ? (u = C.top - o, o = C.top) : u = 0, x(s, p, D, C.left, o, C.width, l), u > 0 && (D.top += u), o = Math.floor(o + p.height) - u
                        }
                    else
                        E("html2canvas: Error loading background:" + o)
            }
            switch (b.nodeName) {
                case "IMG":
                    r = b.getAttribute("src");
                    (F = k(r)) ? (r = a(b, "paddingLeft"), w = a(b, "paddingTop"), p = a(b, "paddingRight"), D = a(b, "paddingBottom"), s.drawImage(F, 0, 0, F.width, F.height, f + r + m[3].width, B + w + m[0].width, j.width - (m[1].width + 
                    m[3].width + r + p), j.height - (m[0].width + m[2].width + w + D)), H += 1) : E("html2canvas: Error loading <img>:" + r);
                    break;
                case "INPUT":
                ...
                case "LI":
                    ...
                    if (/^(decimal|decimal-leading-zero|upper-alpha|upper-latin|upper-roman|lower-alpha|lower-greek|lower-latin|lower-roman)$/i.test(m)) {
                        ...
                        if (f === "inside")
                            R.setVariable("textAlign", "left"), j = j.left, f = r.bottom, B = R, h(F).length > 0 && (B.fillText(F, 
                            j, f), H += 1)
                    }
                    break;
                case "CANVAS":
        ...
        function G(a, b) {
            if (y(a, "display") !== "none" && y(a, "visibility") !== "hidden" && (b = K(a, b) || b, R = b.ctx, !A.test(a.nodeName))) {
                var c = i.Util.Children(a), d, e, j;
                d = 0;
                for (j = c.length; d < j; d += 1)
                    e = c[d], e.nodeType === 1 ? G(e, b) : e.nodeType === 
                    3 && f(a, e, b)
            }
        }
        t.scroll(0, 0);
        var B = {rangeBounds: !1,svgRendering: g.svgRendering && function() {
                var a = new Image, b = u.createElement("canvas"), c = b.getContext === l ? !1 : b.getContext("2d");
                if (c === !1)
                    return !1;
                b.width = b.height = 10;
                ...
                return !0
            }()}, j = g.elements === l ? u.body : g.elements[0], H = 0, w = {}, z = j.ownerDocument, A = RegExp("(" + g.ignoreElements + ")"), O = z.body, M, N, S, R, L, c = c || {};
        ...
        B.svgRendering && function(a) {
            function b(a) {
                ...
                d, e, g, f, h;
                for (h = 0; h < c; h += 1)
                    if (f = a[h], f.nodeType === 3)
                        j += f.nodeValue.replace(/\</g, "&lt;").replace(/\>/g, "&gt;");
            ...
            var c = new Image, d = e(), j = "";
            b(a);
            c.src = ["data:image/svg+xml,", "<svg xmlns='http://www.w3.org/2000/svg' version='1.1' width='" + 
                d.width + "' height='" + d.height + "'>", "<foreignObject width='" + d.width + "' height='" + d.height + "'>", "<html xmlns='http://www.w3.org/1999/xhtml' style='margin:0;'>", j.replace(/\#/g, "%23"), "</html></foreignObject></svg>"].join("");
            c.onload = function() {
                S.svgRender = c
            }
        }(u.documentElement);
    ...
    i.Preload = function(c) {
        ...
        function a(c) {
            var d = i.Util.Children(c), e, h, k = !1;
            try {
                var n = d.length;
                for (e = 0; e < n; e += 1)
                    a(d[e])
        ...
        var b = {numLoaded: 0,numFailed: 0,numTotal: 0,cleanupDone: !1}, h, f, k, n = 0;
        k = c.elements[0] || u.body;
        var Q = k.ownerDocument, P = Q.images, v = P.length, x = Q.createElement("a"), K = function(a) {
            return a.crossOrigin !== l
        }(new Image), G;
        x.href = t.location.href;
        h = x.protocol + x.host;
        f = {loadImage: function(a) {
                var f, g;
                if (a && b[a] === l)
                    f = new Image, a.match(/data:image\/.*;base64,/i) ? (f.src = a.replace(/url\(['"]{0,}|['"]{0,}\)$/ig, 
                    ""), g = b[a] = {img: f}, b.numTotal++, d(f, g)) : (x.href = a, x.href = x.href, x.protocol + x.host === h || c.allowTaint === !0 ? (g = b[a] = {img: f}, b.numTotal++, d(f, g), f.src = a) : K && !c.allowTaint && c.useCORS ? (f.crossOrigin = "anonymous", g = b[a] = {img: f}, b.numTotal++, d(f, g), f.src = a, f.customComplete = function() {
                        if (this.img.complete)
                            this.img.onerror();
                        else
                            this.timer = t.setTimeout(this.img.customComplete, 100)
                    }.bind(g), f.customComplete()) : c.proxy && (g = b[a] = {img: f}, b.numTotal++, e(a, f, g)))
            },cleanupDOM: function(a) {
                var d, e;
                if (!b.cleanupDone) {
                    a && 
                    typeof a === "string" ? E("html2canvas: Cleanup because: " + a) : E("html2canvas: Cleanup after timeout: " + c.timeout + " ms.");
                    for (e in b)
                    ...
                    t.stop !== 
                    l ? t.stop() : u.execCommand !== l && u.execCommand("Stop", !1);
                    u.close !== l && u.close();
                    b.cleanupDone = !0;
                    a && typeof a === "string" || g()
                }
        ...
        for (k = 0; k < v; k += 1)
            f.loadImage(P[k].getAttribute("src"));
        b.firstRun = !1;
    ...
    function(c, g) {
        function e(c) {
            var b = [], g = [], c = c.children, f, k, i, l;
            f = 0;
            for (i = c.length; f < i; f += 1)
                k = c[f], k.children && k.children.length > 0 ? (b.push(k), g.push(k.zindex)) : a.push(k);
            g.sort(function(a, b) {
                return a - b
            });
            c = 0;
        ...
        var a = [];
        e(c.zIndex);
        if (typeof g._renderer._create !== "function")
            throw Error("Invalid renderer defined");
        return g._renderer._create(c, g, u, a, i)
    };
    L = function(c, g) {
        var e, a, d = {logging: !1,
            elements: c,proxy: "http://html2canvas.appspot.com/",timeout: 0,useCORS: !1,allowTaint: !1,svgRendering: !1,iframeDefault: "default",ignoreElements: "IFRAME|OBJECT|PARAM",useOverflow: !0,letterRendering: !1,flashcanvas: l,width: null,height: null,taintTest: !0,renderer: "Canvas"}, d = i.Util.Extend(g, d);
        if (typeof d.renderer === "string" && i.Renderer[d.renderer] !== l)
            d._renderer = i.Renderer[d.renderer](d);
        else if (typeof d.renderer === "function")
            d._renderer = d.renderer(d);
        else
            throw "Unknown renderer";
        i.logging = d.logging;
        d.complete = 
        function(b) {
            if (!(typeof d.onpreloaded === "function" && d.onpreloaded(b) === !1) && (e = i.Parse(b, d), !(typeof d.onparsed === "function" && d.onparsed(e) === !1) && (a = i.Renderer(e, d), typeof d.onrendered === "function")))
                d.onrendered(a)
        };
        t.setTimeout(function() {
            i.Preload(d)
        }, 0);
        return {render: function(a, c) {
    ...
    i.Renderer.Canvas = function(c) {
        var c = 
        c || {}, g = c.canvas || u.createElement("canvas"), e = !1, a = !1, d = !1, b;
        if (g.getContext)
            E("html2canvas: Renderer: using canvas renderer"), d = !0;
            ...
            h.onload = function(a, b) {
                var c;
                if (a.onload === l)
                    a.onreadystatechange !== l ? (c = function() {
        ...
        return b = {_create: function(b, c, h, i, t) {
                if (!d)
                    return a = arguments, g;
                var v = g.getContext("2d"), x, K, G, B, j, H, w = u.createElement("canvas");
                j = w.getContext !== l;
                var z, A, w = j ? w.getContext("2d") : {};
                H = [];
                g.width = g.style.width = !e ? c.width || b.ctx.width : Math.min(2880, c.width || b.ctx.width);
                g.height = g.style.height = 
                !e ? c.height || b.ctx.height : Math.min(2880, c.height || b.ctx.height);
                x = v.fillStyle;
                v.fillStyle = b.backgroundColor;
                v.fillRect(0, 0, g.width, g.height);
                v.fillStyle = x;
                if (c.svgRendering && b.svgRender !== l)
                    v.drawImage(b.svgRender, 0, 0);
                else {
                    K = 0;
                    for (G = i.length; K < G; K += 1) {
                        ...
                        if (x.ctx.storage) {
                            B = 0;
                            for (z = x.ctx.storage.length; B < z; B += 1)
                                switch (A = 
                                x.ctx.storage[B], A.type) {
                                    ...
                                    case "function":
                                        if (A.name === "fillRect")
                                            (!e || A.arguments[0] + A.arguments[2] < 2880 && A.arguments[1] + A.arguments[3] < 2880) && v.fillRect.apply(v, A.arguments);
                                        else if (A.name === "fillText")
                                            (!e || A.arguments[1] < 2880 && A.arguments[2] < 2880) && v.fillText.apply(v, A.arguments);
                ...
                G = c.elements.length;
                return G === 1 && typeof c.elements[0] === "object" && c.elements[0].nodeName !== "BODY" && e === !1 ? (H = t.Util.Bounds(c.elements[0]), j = h.createElement("canvas"), j.width = H.width, j.height = H.height, v = j.getContext("2d"), v.drawImage(g, H.left, H.top, H.width, H.height, 0, 0, H.width, 
                H.height), g = null, j) : g
            }}
    };
    t.html2canvas = L
